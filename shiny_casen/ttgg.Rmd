...
```{r}
# dataset <- read.csv(file = 'Casen2017.csv')
datasett <- read.csv('Casen2017_dos_comunas.csv')
#dataset <- data.frame(datasett)
colnames(datasett)
typeof(colnames(datasett))
```
...
```
Una simple tabulación cruzada, similar a la función base R 'table'

```{r echo = FALSE, error=FALSE, warning=FALSE, message=FALSE}
# tabla_cro <- cro(dataset$sexo, dataset$ecivil)
# tabla_cro
...
Esta tabla nos entrega porcentajes con tablas puestas una al lado de la otra. En este caso primero los sexos y luego los estados civiles


```{r echo = FALSE, error=FALSE, warning=FALSE, message=FALSE}
tabla_cro_cpct <- cro_cpct(dataset$pobreza_multi_4d, list(total(),dataset$sexo, dataset$ecivil))
tabla_cro_cpct
...
el mismo resultado con otra notacion


```{r echo = FALSE, error=FALSE, warning=FALSE, message=FALSE}

dataset %>% calc_cro_cpct(pobreza_multi_4d, list(total(), sexo, ecivil))
#dataset$"r3"
```

# Tabla con columnas anidadas: estado civil bajo sexo

```{r echo = FALSE, error=FALSE, warning=FALSE, message=FALSE}

dataset %>% calc_cro_cpct(pobreza_multi_4d, list(total(), sexo %nest% ecivil))
```
# Tabla con columnas anidadas: pareja bajo estado civil bajo sexo

```{r echo = FALSE, error=FALSE, warning=FALSE, message=FALSE}

#hhh <- dataset %>% calc_cro_cpct(yaut, list(total(), sexo %nest% ecivil))

#hhh
#hhh_df <- data.frame(hhh)
#hhh_df 
...
```{r echo = FALSE, error=FALSE, warning=FALSE, message=FALSE}
#cro_cpct(dataset$sexo, list(total(), dataset$ecivil, dataset$r3, dataset$pobreza_multi_4d))
```
